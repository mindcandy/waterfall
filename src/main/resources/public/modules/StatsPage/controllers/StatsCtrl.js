define([], function () {
    var StatsCtrl = function($scope, $http, $timeout) {
        ...
        $scope.fetchJobs = function () {
            ...
            for (var i = 0, len = testJobs.jobs.length; i < len; i++) {
        ...
        function fetchLogs(jsonJob) {
            jsonJob.logData = $.grep(testLogs.logs, function (e) { return e.jobID == jsonJob.jobID; });
            ...
            for (var k = 0, kLen = seriesData.length; k < kLen; k++) {
                for (var m = 0, mLen = seriesData[k].data.length; m < mLen; m++) {
                    ...
                    for (var n = 0, nLen = dateRange.length; n < nLen; n++) {
                        if (dateRange[n].format('YYYY-MM-DD') === seriesData[k].data[m][0]) {
                            index = n;
                            break;
                        }
                    }
        ...
        function getTotalRuntimeSeriesData(seriesData) {
            ...
            for (var i = 0, iLen = i < dateRange.length; i < iLen; i++) {
                ...
                for (var j = 0, jLen = seriesData.length; j < jLen; j++) {
                    if (dateRange[i].format('YYYY-MM-DD') === seriesData[j].date) {
                        containsDate = true;
                        break;
                    }
                }
                if (!containsDate) {
                    seriesData.push({ date: dateRange[i].format('YYYY-MM-DD'), runtime: 0 });
                }
            }
        ...
        function getDropRuntimesChart() {
            ...
            for (var i = 0, iLen = $scope.jobCount; i < iLen; i++) {
                ...
                for (var j = 0, jLen = $scope.jobs[i].logData.length; j < jLen; j++) {
                    ...
                    job.data.push([ start.format('YYYY-MM-DD'), end.unix() - start.unix() ]);
                }
                dataArray.push(job);
            }
        ...
        function getTotalRuntimesChart() {
            ...
            for (var i = 0, iLen = $scope.jobCount; i < iLen; i++) {
                for (var j = 0, jLen = $scope.jobs[i].logData.length; j < jLen; j++) {
                    ...
                    var jobRuntime = moment(jobLogs.endTime).unix() - moment(jobLogs.startTime).unix();
